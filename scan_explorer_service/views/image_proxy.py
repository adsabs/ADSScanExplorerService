...
def image_proxy(path):
    ...
    current_app.logger.debug(f'Request method: {request_method}, Request url: {req_url}, Params: {params}, Headers: {req_headers}')
...
def pdf_save():
    ...
    try:
        ...
        @stream_with_context
        def loop_images(session, item, page_start, page_end, page_limit):
            n_pages = 0
            memory_sum = 0
